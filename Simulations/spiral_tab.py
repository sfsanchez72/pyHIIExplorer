...
blobs_final,blobs_F_Ha,image_HII, diff_map_final,diff_points,diff_Flux = HIIblob(img_Ha_ALL,img_cont_Ha,FWHM, MUSE_1sig=1.0, MUSE_1sig_V=0.1,
                                                                                 plot=plot, refined=3, num_sigma=300, name=name, max_size=2, DIR=DIR)
...
cube_diff_output[4,:,:] = (cube_diff_output[2,:,:]/hdu_cube_ALL.data[2,:,:])*hdu_cube_ALL.data[4,:,:] #modificando EW
...
#################################
    ...
    #plt.axvline(-0.4, 0.27, c='#646464', ls="--", linewidth=2)
    #plt.axhline(0.79, 0.37, 1, c='#646464', ls="--", linewidth=2)
    #plt.axhline(0.5, c='#646464', ls="--", linewidth=2)
    
    #plt.text(-1, 3., 'SF', fontsize=35, fontweight=800)
    #plt.text(0.3, 3., 'sAGN', fontsize=35, fontweight=800)
...
table_info=Table(rows=[(r, PA, ab, A, B, C, N_sp, x_c, y_c, size, FWHM, spax_scale, cont_peak, np.around(percentage_fleak*100,2), len(blobs_in), len(blobs_final), np.around(np.float((100*len(blobs_final))/len(blobs_in)), 2), np.around(np.float((100*len(blobs_final[mask_flux_05]))/len(blobs_in[mask_flux_05_in])),2), np.around(np.float(np.mean(((Ha_HII_out-Ha_HII_inp)/Ha_HII_inp)[Ha_HII_inp>0.5*np.nanmedian(Ha_HII_inp[Ha_HII_inp>0])])),2), np.around(np.float(np.std(((Ha_HII_out-Ha_HII_inp)/Ha_HII_inp)[Ha_HII_inp>0.5*np.nanmedian(Ha_HII_inp[Ha_HII_inp>0])])),2), np.around(np.float(np.mean(R_in[mask_flux_rad05])),2), np.around(np.float(np.std(R_in[mask_flux_rad05])),2), np.around(np.float(np.mean(blobs_final[:,2][mask_flux_bl05])),2), np.around(np.float(np.std(blobs_final[:,2][mask_flux_bl05])),2), np.around(np.float(np.mean(N2_HII_out[mask_N2_HII]-N2_HII_inp[mask_N2_HII])),2), np.around(np.float(np.std(N2_HII_out[mask_N2_HII]-N2_HII_inp[mask_N2_HII])),2), np.around(np.float(np.mean(O3_HII_out[mask_O3_HII]-O3_HII_inp[mask_O3_HII])),2), np.around(np.float(np.std(O3_HII_out[mask_O3_HII]-O3_HII_inp[mask_O3_HII])),2), np.around(np.float(np.mean(((ew_HII_out-ew_HII_inp)/ew_HII_inp)[mask_ew_HII])),2), np.around(np.float(np.std(((ew_HII_out-ew_HII_inp)/ew_HII_inp)[mask_ew_HII])),2), np.around(np.float(np.mean(N2_DIG_out[mask_N2_DIG]-N2_DIG_inp[mask_N2_DIG])),2), np.around(np.float(np.std(N2_DIG_out[mask_N2_DIG]-N2_DIG_inp[mask_N2_DIG])),2), np.around(np.float(np.mean(O3_DIG_out[mask_O3_DIG]-O3_DIG_inp[mask_O3_DIG])),2), np.around(np.float(np.std(O3_DIG_out[mask_O3_DIG]-O3_DIG_inp[mask_O3_DIG])),2), np.around(np.float(np.mean(((ew_DIG_out-ew_DIG_inp)/ew_DIG_inp)[mask_ew_DIG])),2), np.around(np.float(np.std(((ew_DIG_out-ew_DIG_inp)/ew_DIG_inp)[mask_ew_DIG])),2)  )], names=('r', 'PA', 'ab', 'A', 'B', 'C', 'arms', 'cen_x', 'cen_y', 'size', 'FWHM', 'spax_scale', 'cont_peak', '%f_leak', 'inp_hiiregions', 'out_hiiregions', '%_recovery', '%_recovery_05', 'delta_Ha_05', 'std_delta_Ha_05', 'mean_inp_R_05', 'std_inp_R_05', 'mean_out_R_05', 'std_out_R_05', 'mean_delta_N2_hiiregions', 'std_delta_N2_hiiregions', 'mean_delta_O3_hiiregions', 'std_delta_03_hiiregions', 'mean_delta_ew_hiiregions', 'std_delta_ew_hiiregions', 'mean_delta_N2_dig', 'std_delta_N2_dig', 'mean_delta_O3_dig','std_delta_O3_dig','mean_delta_ew_dig', 'std_delta_ew_dig' )  )


file_table_info = DIR+'/table_'+name+'.table.ecsv'

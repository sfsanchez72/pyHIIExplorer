...
def extracting_flux_elines(name, hdu, blobs_final, diff_points, FWHM_MUSE, plot=0, def_DIG=0, cube_DIG=np.array):
...
def extracting_ssp(name, hdu_ssp, WCS, blobs_final, diff_points, FWHM_MUSE, plot=0):
    desc_ssp = ["V", "CS", "DZ", "flux", "e_flux", "age_LW", "age_MW", "e_age", "ZH_LW", "ZH_MW", "e_ZH", "Av", "e_Av","vel_SSP", "e_vel_SSP", "disp_SSP", "e_disp_SSP", "ML", "mass", "mass_dust", "e_mass"]

    #data_SSP=hdu_ssp[0].data
    #hdr_ssp=hdu_ssp[0].header
    
    try:
        data_SSP = hdu_ssp.data
        hdr_ssp = hdu_ssp.header
...
def extracting_sfh(name, hdu_sfh, WCS, blobs_final, diff_points, FWHM_MUSE, plot=0):
    #data_SFH=hdu_sfh[0].data
    #hdr_sfh=hdu_sfh[0].header
    
    try:
        data_SFH = hdu_sfh.data
        hdr_sfh = hdu_sfh.header
    ...
    #print(key_now)
        ...
        if (plot == 1):
            ...
            im_Ha=axes[0].imshow(map_now, interpolation='none',cmap=cmap, norm=colors.PowerNorm(gamma=0.5))
            clim=im_Ha.properties()['clim']
            axes[1].imshow(img_HII, interpolation='none',cmap=cmap, label=r'H$\alpha$',
                           norm=colors.PowerNorm(gamma=0.5), clim=clim)
            axes[2].imshow(img_diff, interpolation='none',cmap=cmap, label=r'H$\alpha$',
                           norm=colors.PowerNorm(gamma=0.5), clim=clim)
...
def extracting_index(name, hdu_index, WCS, blobs_final, diff_points, FWHM_MUSE, plot=0):
    ...
    try:
        data_INDEX = hdu_index.data
        hdr_index = hdu_index.header

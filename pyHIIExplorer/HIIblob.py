...
# g-band
 ...
 #   (nx_m,ny_m)=X_sqr.shape
    ...
    model_map_masked = np.ma.array(model_map, mask = (model_map<1.5*MUSE_1sig), fill_value=0.0)
    model_map = model_map_masked.filled()
    
    image_HII_masked = np.ma.array(image_HII, mask = (image_HII<1.5*MUSE_1sig), fill_value=0.0)
    image_HII = image_HII_masked.filled()
    
    diff_map_masked = np.ma.array(diff_map, mask = (diff_map<1.5*MUSE_1sig), fill_value=0.0)
    diff_map = diff_map_masked.filled()
    
    res_map = F_Ha_MUSE-model_map
...
#                           marker='+',edgecolor='blue',color='black',alpha=0.3)
        ...
        clim=im_Ha_MUSE.properties()['clim']
        im_mod=axes[0][1].imshow(model_map, interpolation='none',\
                                 norm=colors.PowerNorm(gamma=0.5), cmap=cmap,clim=clim, label=r'HII regions + DIG models') 
       
                               
        im_HII_mod=axes[1][0].imshow(image_HII, interpolation='none',\
                                     norm=colors.PowerNorm(gamma=0.5),cmap=cmap,\
                                     clim=clim, label=r'HII regions model') 
        im_diff=axes[1][1].imshow(diff_map, interpolation='none',\
                                  norm=colors.PowerNorm(gamma=0.5), cmap=cmap, \
                                  clim=clim, label=r'DIG model')
                                     
        im_res=axes[0][2].imshow(res_map, interpolation='none',\
                                 cmap='RdBu', \
                                 vmin=-2*desvi, vmax=2*desvi, label=r'Residue')
                                     
        im_res_rela=axes[1][2].imshow(res_map/F_Ha_MUSE, interpolation='none',\
                                      cmap='RdBu',\

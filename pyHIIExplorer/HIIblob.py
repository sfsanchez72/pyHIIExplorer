...
#        print('Blobs near =',blobs[ind_blobs])
        ...
        image_sec_model=create_HII_image(blobs_near,flux_input_near,ny_sec,nx_sec,dr=dr)
...
def create_HII_image(blobs,flux,ny,nx,dr=5):
    image = np.zeros((ny,nx))
...
def delta_HII_image_to_fit(org_img,blobs,ny,nx,dr=5): 
    def delta_img(x, *flux): 
        model_img=create_HII_image(blobs,flux,ny,nx,dr=dr)
        delta = np.nansum((org_img-model_img)**2/org_img)
        return delta
    return delta_img

def HII_image_to_fit(blobs,ny,nx,dr=5): 
    def delta_img(x, *flux): 
        model_img=create_HII_image(blobs,flux,ny,nx,dr=dr)
        return model_img
    return delta_img

def flatten_HII_image_to_fit(blobs,ny,nx,dr=5): 
    def delta_img(x, *flux): 
        model_img=create_HII_image(blobs,flux,ny,nx,dr=dr)
        return model_img.flatten()
    return delta_img

def ravel_HII_image_to_fit(blobs,ny,nx,dr=5): 
    def delta_img(x, *flux): 
        model_img=create_HII_image(blobs,flux,ny,nx,dr=dr)
...
#    print("LEN_DIFF = ",len(points_MUSE)," DIFF=",points_MUSE)
    if(len(points_MUSE)>3):
        ...
        xgrid=np.arange(0, nx, 1)
        ygrid=np.arange(0, ny, 1)
...
def HIIextraction(Ha_image,blobs_log_MUSE,kind=0,we=2,\
    ...
    #
    blobs_temp = blobs_log_MUSE.copy()
        
    blobs_temp[:,2] = blobs_temp[:,2]/np.sqrt(2)
        
    image_HII=create_HII_image(blobs_temp,blobs_F_Ha,ny,nx)
...
def HIIblob(F_Ha_MUSE,V_MUSE,FWHM_MUSE, MUSE_1sig=0, MUSE_1sig_V=0, plot=0, refined=0, name="NONE", DIG_lim=1, num_sigma=30, max_size=1.0, DIR='none'):
...
# g-band
    ...
    diff_map,diff_points,diff_Flux = create_diff(F_Ha_MUSE,blobs_log_MUSE,FWHM_MUSE)  #
    #diff_map,diff_points,diff_Flux = create_diff(F_Ha_MUSE-image_HII,blobs_log_MUSE,FWHM_MUSE)   #sobresustrae el flujo de las regiones HII
    ...
    diff_map,diff_points,diff_Flux = create_diff(np.abs(F_Ha_MUSE-image_HII),blobs_log_MUSE,FWHM_MUSE)
    diff_map = create_diff_new(np.abs(F_Ha_MUSE-image_HII),blobs_log_MUSE,FWHM_MUSE, diff_points)   
    res_map = F_Ha_MUSE-(image_HII+diff_map)                                                        
    ...
    DIG_blobs_F_Ha,DIG_image_HII,DIG_diff_map_final,DIG_diff_points,DIG_diff_Flux=HIIextraction(diff_map,blobs_final,\
                                                                                                kind=0,we=2,\
                                                                                                FWHM_MUSE = FWHM_MUSE, refined = 0)
 ...
 #   (nx_m,ny_m)=X_sqr.shape
    ...
    # final mask for images
    
    mask_F_Ha = mask_MUSE
    
    model_map_res = model_map.copy()
    
    model_map_res_masked = np.ma.array(model_map_res, mask = mask_F_Ha , fill_value=0.0)
    model_map_res_masked = model_map_res_masked.filled()
    
    image_HII_masked = np.ma.array(image_HII, mask = mask_F_Ha, fill_value=0.0)
    image_HII_masked = image_HII_masked.filled()
    
    diff_map_masked = np.ma.array(diff_map, mask = mask_F_Ha, fill_value=0.0)
    diff_map_masked = diff_map_masked.filled()
    
    #mask only residues!!
    
    res_map_r = res_map.copy()
    res_map_masked = np.ma.array(res_map_r, mask = mask_F_Ha, fill_value=0.0)
    res_map_masked = res_map_masked.filled()
    
    res_map_relative = res_map_r/F_Ha_MUSE
    res_map_relative_masked = np.ma.array(res_map_relative, mask = mask_F_Ha, fill_value=0.0)
    res_map_relative_masked = res_map_relative_masked.filled()
...
#        plt.ion()
        ...
        cmap='gist_stern_r'
        im_Ha_MUSE=axes.imshow(F_Ha_MUSE, interpolation='none',\
                                     cmap=cmap,
                                     norm=colors.PowerNorm(gamma=0.5), label=r'Original H$\alpha$ map')
                                     #vmin=-2*desvi, vmax=median+3*desvi)
        
        #cb1 = fig.add_axes([0.91, 0.52, 0.01, 0.38])
        #cb1.tick_params(labelsize=20)
        cb1=fig.colorbar(im_Ha_MUSE)
        cb1.set_label(label='flux [10$^{-16}ergs^{-1}cm^{-2}arcsec^{-2}$]',fontsize=20)                             
                                     
        for blob in blobs_final:
            y, x, r = blob
            c = plt.Circle((x, y), r, color='darkred', linewidth=2, fill=False, alpha=0.4)
            axes.add_patch(c)
        ...
        fileout_png = "DETECT."+name+".png"
        fig.savefig(DIR+'/'+fileout_png, transparent=False, facecolor='white', edgecolor='white')
        
        
        fig, axes = plt.subplots(2,3, figsize=(22,15))
        cmap='gist_stern_r'
        median = np.nanmedian(F_Ha_MUSE)
        desvi = np.nanstd(F_Ha_MUSE)
        ...
        #for blob in blobs_final:
        #    y, x, r = blob
        #    c = plt.Circle((x, y), r, color='black', linewidth=2, fill=False, alpha=0.4)
        #    axes[0][0].add_patch(c)
...
#                           marker='+',edgecolor='blue',color='black',alpha=0.3)
        ...
        clim=im_Ha_MUSE.properties()['clim']
        im_mod=axes[0][1].imshow(model_map_res_masked, interpolation='none',\
                                 norm=colors.PowerNorm(gamma=0.5), cmap=cmap,clim=clim, label=r'HII regions + DIG models') 
       
                               
        im_HII_mod=axes[1][0].imshow(image_HII_masked, interpolation='none',\
                                     norm=colors.PowerNorm(gamma=0.5),cmap=cmap,\
                                     clim=clim, label=r'HII regions model') 
        im_diff=axes[1][1].imshow(diff_map_masked, interpolation='none',\
                                  norm=colors.PowerNorm(gamma=0.5), cmap=cmap, \
                                  clim=clim, label=r'DIG model')
                                     
        im_res=axes[0][2].imshow(res_map_masked, interpolation='none',\
                                 cmap='RdBu', \
                                 vmin=-2*desvi, vmax=2*desvi, label=r'Residue')
                                     
        im_res_rela=axes[1][2].imshow(res_map_relative_masked, interpolation='none',\
                                      cmap='RdBu',\
                                      vmin=-1, vmax=1, label=r'Relative residue')
                                      
                                 
        cb1 = fig.add_axes([0.91, 0.52, 0.01, 0.38])
        cb1.tick_params(labelsize=20)
        cb1=fig.colorbar(im_res, cax=cb1)
        cb1.set_label(label='$\Delta$flux [10$^{-16}ergs^{-1}cm^{-2}arcsec^{-2}$]',fontsize=30)
        
        cb2 = fig.add_axes([0.91, 0.12, 0.01, 0.38])
        cb2.tick_params(labelsize=20)
        cb2=fig.colorbar(im_res_rela, cax=cb2)
        cb2.set_label(label='$\Delta$flux/flux',fontsize=30)
        
        cb3 = fig.add_axes([0.125, 0.085, 0.52, 0.02])
        cb3.tick_params(labelsize=20)
        cb3=fig.colorbar(im_Ha_MUSE, cax=cb3, orientation='horizontal')
        cb3.set_label(label='flux [10$^{-16}ergs^{-1}cm^{-2}arcsec^{-2}$]',fontsize=30)
        
        #axes[0][0].set_title(r'Original H$\alpha$ map', fontsize=32)
        #axes[0][1].set_title(r'Candidates + DIG models', fontsize=32)
        #axes[0][2].set_title(r'Residue', fontsize=32)
        #axes[1][0].set_title(r'Candidates model', fontsize=32)
        #axes[1][1].set_title(r'DIG model', fontsize=32)
        #axes[1][2].set_title(r'Relative residue', fontsize=32)
        
        axes[0][0].text(0.14, 0.83, r'Original H$\alpha$ map', fontsize=30, bbox=dict(facecolor='white', alpha=0.35), fontweight=800,transform=fig.transFigure)
        axes[0][1].text(0.4, 0.83, r'Candidates + DIG models', fontsize=30,bbox=dict(facecolor='white', alpha=0.35), fontweight=800, transform=fig.transFigure)
        axes[0][2].text(0.67, 0.83, r'Residue', fontsize=30, bbox=dict(facecolor='white', alpha=0.35), fontweight=800, transform=fig.transFigure)
        axes[1][0].text(0.14, 0.43, r'Candidates model', fontsize=30, bbox=dict(facecolor='white', alpha=0.35), fontweight=800, transform=fig.transFigure)
        axes[1][1].text(0.4, 0.43, r'DIG model', fontsize=30, bbox=dict(facecolor='white', alpha=0.35), fontweight=800, transform=fig.transFigure)
        axes[1][2].text(0.67, 0.43, r'Relative residue', fontsize=30, bbox=dict(facecolor='white', alpha=0.35), fontweight=800, transform=fig.transFigure)
        ...
        axes[0][2].set_ylim(0,ny)
        axes[1][2].set_ylim(0,ny)
        
        plt.subplots_adjust(wspace=0.005, hspace=0.1) 
        
        axes[0][0].axes.yaxis.set_ticklabels([])  
        axes[0][1].axes.yaxis.set_ticklabels([])
        axes[1][0].axes.yaxis.set_ticklabels([])
        axes[1][1].axes.yaxis.set_ticklabels([])
        axes[0][2].axes.yaxis.set_ticklabels([])
        axes[1][2].axes.yaxis.set_ticklabels([])
    ...
    #blobs_final[:, 2] = blobs_final[:, 2]/0.9
    return blobs_final,blobs_F_Ha,image_HII,diff_map,diff_points,diff_Flux
